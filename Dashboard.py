...
with aba1:
    ...
    with coluna2:
        ...
        st.plotly_chart(fig_receita_categorias, use_containers = True)

with aba2:
    coluna1, coluna2 = st.columns(2)
    with coluna1:
        st.metric('Receita', formata_numero(dados['Preço'].sum(), 'R$'))
        st.plotly_chart(fig_mapa_vendas, use_containers_width = True)
        st.plotly_chart(fig_vendas_estados, use_containers_width = True)
    with coluna2:
        st.metric('Quantidade de vendas', formata_numero(dados.shape[0]))
        st.plotly_chart(fig_vendas_mensal, use_containers_width = True)
        st.plotly_chart(fig_vendas_categorias, use_containers_width = True)
